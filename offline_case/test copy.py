...
def colorize_depth(depth: np.ndarray, d_min: int, d_max: int) -> np.ndarray:
    """
    TURBO colormap using the real capture depth range from info.txt
    so the color scale is consistent with what main.py produced live.
    """
    if d_max > d_min:
        clipped = np.clip(depth.astype(np.float32), d_min, d_max)
        normalized = ((clipped - d_min) / (d_max - d_min) * 255).astype(np.uint8)
    else:
        normalized = cv2.normalize(depth, None, 0, 255, cv2.NORM_MINMAX).astype(
            np.uint8
        )
...
def run(name: str, base: Path, save: bool):
    ...
    # 5. Colorize depth using the real range from info.txt
    depth_colored = colorize_depth(depth, info.depth_min, info.depth_max)
